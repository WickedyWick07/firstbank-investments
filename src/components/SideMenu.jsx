...
const SideMenu = () => {
  ...
  return (
    <div className={`relative flex`}>
      ...
      {/* Side Menu */}
      <nav className={`fixed inset-0 bg-black bg-opacity-75 z-50 transition-all duration-300 ${isOpen ? 'block' : 'hidden md:block'}`}>
        <div className={`border-r border-primaryGold px-4 w-64 flex flex-col h-full`}>
          {/* Close Button Inside the Menu */}
          <button 
            onClick={toggleMenu} 
            className='self-end p-2 bg-primaryGold text-white rounded'>
            <i className="bi bi-x"></i> {/* Close icon */}
          </button>
